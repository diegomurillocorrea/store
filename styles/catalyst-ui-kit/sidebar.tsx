...
export function SidebarBody({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex flex-1 flex-col overflow-y-auto p-4 [&>[data-slot=section]+[data-slot=section]]:mt-5'
      )}
    />
  )
}
...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  let classes = clsx(
    // Base
    'flex w-full items-center gap-2.5 rounded-lg px-2 py-1.5 text-left text-sm/5 font-medium text-foreground',
    // Leading icon/icon-only
    '*:data-[slot=icon]:size-5 *:data-[slot=icon]:shrink-0 *:data-[slot=icon]:fill-foreground/70',
